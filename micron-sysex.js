...
export function parsePatchDump(data) {
  ...
  const patch = unpacked.slice(0);
  const nameBytes = patch.slice(63, 78);
  const name = nameBytes.map(b => b>=32&&b<127 ? String.fromCharCode(b) : '').join('').trim() || 'Untitled';
  const params = extractParams(patch);
  return { name, bank, slot, params, raw: patch };
}
...
export function extractParams(p) {
  return {
    ...
    fx2ParamsE: p[354], fx2ParamsF: p[355],
